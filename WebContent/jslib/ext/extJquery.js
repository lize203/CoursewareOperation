...
$.ajaxSetup({
	...
	error : function(XMLHttpRequest, textStatus, errorThrown) {
		try {
			...
			//parent.$.messager.alert('错误','<div>请求异常<div style="display:none;">'+XMLHttpRequest.responseText+'</div></div>');
		} catch (e) {
...
function progressLoad(_maskStyle,_tipStyle){
	// var maskStyle = "position:fixed;widht:100%;height:100%;top:0;left:0;";  
	// var tipStyle = "top:50%;left:50%;margin-top:-16px;margin-left:-48px;position:fixed;";
	var maskStyle = _maskStyle?_maskStyle:'';
	var tipStyle = _tipStyle?_tipStyle:'';
    $("<div class=\"datagrid-mask\" style=\"position:absolute;z-index: 9999;"+maskStyle+"\"></div>").css({display:"block",width:"100%",height:$(window).height()}).appendTo("body");  
    $("<div class=\"datagrid-mask-msg\" style=\"position:absolute;z-index: 9999;"+tipStyle+"\"></div>").html("正在处理，请稍候。。。").appendTo("body").css({display:"block",left:($(document.body).outerWidth(true) - 190) / 2,top:($(window).height() - 45) / 2});  
}
