...
public class SecurityInterceptor implements HandlerInterceptor
{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception
	{
		...
		SysUser sessionInfo = (SysUser) request.getSession().getAttribute("loginUser");
		if ((sessionInfo == null) || (sessionInfo.getFlid() == null))
		{// 如果没有登录或登录超时
		...
		{
			List<SysMenu> menulist = (List<SysMenu>) request.getSession().getAttribute("topMenus");
			Set<String> menuSet = new HashSet<String>();
			for (SysMenu tbsysmenu : menulist)
			{
				menuSet.add(tbsysmenu.getFlurl());
			}
